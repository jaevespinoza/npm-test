...
var Server = function (port) {
    var server = Percolator({'port': port, 'autoLink': false, 'staticDir': __dirname
            + '/../frontend'});
            ...
            req.onJson(function (err, newKeyword) {
                if (err) {
                    ...
                    dbSession.query('INSERT INTO Keyword (value, categoryID) VALUES (?, ?);',
                        [newKeyword.value, newKeyword.categoryID], function () {
                            if (err) {
                                console.log(err);
                                res.status.internalServerError(err);
                            } else {
                                res.object({'status': 'ok', 'id': res.insertId}).send()
                            }
                    });
                }
            });
            ...
            req.onJson(function(err, keyword) {
                if (err) {
                    ...
                    dbSession.query('UPDATE keyword SET value = ?, categoryID = ? WHERE keyword.id = ?;',
                        ...
                        if (err) {
                            console.log(err);
                            es.status.internalServerError(err);
                        } else {
